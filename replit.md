...
- `pnpm --filter @workspace/api-server run dev` — run the API node (port 5000, auto-mines every 15 s)
...
- **`@noble/ed25519` async only** — do not assign `ed.etc.sha512Sync`; the object is frozen in v2. All key ops use the async API (`getPublicKeyAsync`, `signAsync`) which uses Web Crypto internally.
