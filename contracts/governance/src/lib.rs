//! Equilibrium Governance Contract
//!
//! Token-weighted on-chain governance with quorum, veto, and executable
//! messages. Proposals can update gov_param, call other contracts, or
//! trigger treasury spends.
//!
//! Voting power = bonded governance tokens at vote time (simplified bonded-vote
//! model; future: read staking contract snapshots for stake-weighted voting).
//!
//! Param names consumed via gov_param() host import (must match ChainParameters
//! in artifacts/api-server/src/chain/governance.ts):
//!   govMinDeposit      — minimum proposal deposit (base units)
//!   govVotingPeriod    — voting window in blocks
//!   govQuorumThreshold — absolute minimum total vote weight (base units)
//!   govPassThreshold   — pass fraction × SCALE (e.g. 0.50 × 10^18)
//!
//! gov_pending_param:{key} storage entries are polled by the TypeScript
//! processBlock bridge and applied to live ChainParameters.
//!
...
//!    -> proposal_id (>=0), or -1..-4
//! 1  vote(proposal_id: i64, option: u8)
//!    -> 1 success, 0 already voted, -1 unknown, -2 not open, -3 no power
//! 2  end_voting(proposal_id: i64)
//!    -> 2 passed, 1 failed, 0 not ended, -1 unknown
//! 3  execute_proposal(proposal_id: i64)
//!    -> 1 success, 0 not passed, -1 unknown, -2 already executed, -3 exec fail
//! 4  deposit(proposal_id: i64, amount_fp: i64)
//!    -> 1 success, -1 unknown, -2 below min
//! 5  cancel_proposal(proposal_id: i64)
//!    -> 1 success, 0 not proposer, -1 unknown, -2 already active
//! 6  get_proposal(proposal_id: i64)
//!    -> stores JSON in query_result, returns 1
...
extern "C" {
    ...
    // Signature matches the TypeScript WasmVM host: 5-param form used by all
    // on-chain contracts (see contracts/arbitrage/src/lib.rs for the canonical
    // declaration). addr_ptr/addr_len point to the target contract's address
    // as a UTF-8 hex string; arg_word_count is the number of i32 words at
    // args_ptr (not bytes).
    fn call_contract(
        addr_ptr: *const u8, addr_len: u32,
        method_id: i32,
        args_ptr: u32, arg_word_count: u32,
    ) -> i32;
}
...
// Message types — stored as the first CSV field of each proposal message.
// gov_param:    "0,{param_name},{value_i64}"
// treasury:     "1,{recipient_addr},{amount_i64}"
// contract_call:"2,{contract_addr},{method_id},{arg0},{arg1},..."
...
fn get_voting_power(addr: &str) -> i64 {
    // Simplified: voting power = current bonded governance amount.
    // Future: snapshot at proposal creation block by reading staking contract.
    get_i64(&format!("gov_bond:{}", addr))
}

fn bond_gov_tokens(addr: &str, amount: i64) -> bool {
    if unsafe { bond(amount) } != 1 { return false; }
    let k = format!("gov_bond:{}", addr);
    set_i64(&k, get_i64(&k) + amount);
    true
}

// ── proposal message encoding ────────────────────────────────────────────────
// Messages are stored as comma-separated strings:
//   gov_param:      "0,{param_name},{value}"
//   treasury_spend: "1,{recipient},{amount}"
//   contract_call:  "2,{contract_addr},{method_id},{arg0},{arg1},..."
...
fn execute_message(msg: &str, _proposal_id: i64) -> bool {
    ...
    match msg_type {
        MSG_GOV_PARAM => {
            ...
            let value: i64 = match fields[1].parse() {
                Ok(v) => v,
                Err(_) => return false,
            };
            // Write to gov_pending_param:{name}. The TypeScript processBlock
            // bridge (chain/state.ts) polls these keys each block and applies
            // them to the live ChainParameters — giving the WASM contract a
            // governed path to update any ChainParameters key.
            storage_write(&format!("gov_pending_param:{}", param_name), &value.to_string());
            host_log(&format!("GovParamPending {}={}", param_name, value));
            true
        }
        MSG_TREASURY_SPEND => {
            if fields.len() < 2 { return false; }
            let recipient = fields[0];
            let amount: i64 = match fields[1].parse() {
                Ok(v) => v,
                Err(_) => return false,
            };
            ...
            if ok { host_log(&format!("TreasurySpend {} to {}", amount, recipient)); }
            ok
        }
        MSG_CONTRACT_CALL => {
            if fields.len() < 2 { return false; }
            let contract_addr = fields[0];
            let method_id: i32 = match fields[1].parse() {
                Ok(v) => v,
                Err(_) => return false,
            };
            // Parse remaining fields as i32 argument words.
            ...
            // Call using the 5-param ABI shared by all on-chain contracts.
            // addr_ptr/addr_len: contract address as UTF-8 hex string.
            // args_ptr: pointer to the i32 word array cast to u32.
            // arg_word_count: number of i32 words (not bytes).
            let result = unsafe {
                call_contract(
                    contract_addr.as_ptr(), contract_addr.len() as u32,
                    method_id,
                    arg_words.as_ptr() as u32, arg_words.len() as u32,
                )
            };
...
fn method_submit_proposal(args_ptr: u32) -> i32 {
    ...
    let voting_period = host_gov_param("govVotingPeriod").max(1);
...
fn method_vote(args_ptr: u32) -> i32 {
    ...
    let power = get_voting_power(&caller);
    if power <= 0 { return -3; }
...
fn method_end_voting(args_ptr: u32) -> i32 {
    ...
    let now = unsafe { block_number() } as i64;
    let voting_end = get_i64(&key("proposal_voting_end", proposal_id));
    if now <= voting_end { return 0; } // still open
    ...
    // govQuorumThreshold: absolute minimum total vote weight.
    let quorum_threshold = host_gov_param("govQuorumThreshold");
    // govPassThreshold: fraction × SCALE — yes must exceed pass_threshold% of total.
    ...
    if total < quorum_threshold {
        ...
        host_log(&format!("ProposalFailed id={} reason=quorum total={} required={}", proposal_id, total, quorum_threshold));
        return 1;
    }

    // yes > pass_threshold/SCALE × total  ⟺  fp_mul(yes, SCALE) > fp_mul(pass_threshold, total)
    if fp_mul(yes, SCALE) > fp_mul(pass_threshold, total) {
        ...
        set_i64(&key("proposal_status", proposal_id), STATUS_FAILED);
        host_log(&format!("ProposalFailed id={} reason=threshold yes={} total={}", proposal_id, yes, total));
        return 1;
    }
}
...
fn method_cancel_proposal(args_ptr: u32) -> i32 {
    ...
    let status = get_i64(&key("proposal_status", proposal_id));
    // Can only cancel proposals that are not yet terminal.
    if status != STATUS_PROPOSED && status != STATUS_VOTING { return -2; }
...
fn method_get_proposal(args_ptr: u32) -> i32 {
    ...
    let json = format!(
        "{{\"id\":{},\"proposer\":\"{}\",\"title\":\"{}\",\"desc\":\"{}\",\
         \"status\":{},\"deposit\":{},\"voting_start\":{},\"voting_end\":{},\
         \"yes\":{},\"no\":{},\"abstain\":{},\"executed\":{},\"msg_count\":{}}}",
        id, proposer, title, desc, status, deposit, voting_start, voting_end,
        yes, no, abstain, executed, msg_count
    );
...
fn method_get_vote(args_ptr: u32) -> i32 {
    ...
    let power = get_voting_power(&voter);
...
fn method_get_capabilities(_args_ptr: u32) -> i32 {
    0x1FF // bits 0–8: all nine methods
}
